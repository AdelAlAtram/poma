...
import {
  ...
  VideoCameraIcon,
  PhotoIcon,
} from "@heroicons/react/24/outline"; // Updated to use PhotoIcon
import {
  Dialog,
  DialogContent,
  DialogActions,
...
export default function Blog() {
  ...
  const fetchBlog = async () => {
    try {
      ...
      setBlog(response.data); // Assuming response.data is an array of blog
    } catch (error) {
  ...
  <div
    ...
    className={`flex flex-col sm:flex-row items-center justify-between w-full sm:w-3/4 md:w-2/3 lg:w-1/2 mt-16 ${
      index === blog?.blogs?.length - 1 ? "mb-24" : ""
    }`}
  >
    ...
    <div className="flex gap-3 mt-4 sm:mt-0"> {/* Added mt-4 on mobile */}
